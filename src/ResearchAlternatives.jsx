  ...
  return <section className={`research-alt research-alt--${mode}`} aria-labelledby={`research-${mode}-title`}>
    ...
    <div className="research-alt__items">
      {researches.map(([title,copy],index)=><article className="research-alt__item" key={title}>
        ...
        <div className="research-alt__copy"><h3>{mode === 'shelf' ? <a className="research-alt__title-link" href={`mailto:info@wmt-ai.ru?subject=${encodeURIComponent('Запрос исследования: '+title)}`} aria-label={`Запросить исследование: ${title}`}><span>{title}</span></a> : title}</h3><p>{copy}</p></div>
        {mode === 'shelf' && <div className="research-alt__actions"><a className="button button--dark research-alt__request" href="#contact" aria-label={`Запросить исследование: ${title}`}>Запросить исследование<Arrow arrow compact/></a></div>}
